...
extension Date {
    ...
    static func timestamp() -> Int {
        return Int((Date().timeIntervalSince1970 * 1000.0)) //1000000.0
    }
...
extension UIImageView {
    ...
    public func imageFrom(url: String?) {
        ...
        if let cachedImage = imageCache.object(forKey: urlT as AnyObject) {
            ...
            self.image = nil
            URLSession.shared.dataTask(with: URL(string: urlT)!, completionHandler: { (data, response, error) -> Void in
